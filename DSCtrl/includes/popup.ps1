<#
.SYNOPSIS
Setup of functions that displays a variety of different popup windows.
.DESCRIPTION
Set of functions that list a variety of different popup windows, e.g. -

    PopupListSelect - display a listing that can return the selected item.
    PopupBox - display text in a popup box.
    Choose-ADOrganizationalUnit - choose an organizational unit from a GUI
#>

function PopupListSelect {
    <#
    .SYNOPSIS
    Function takes the provided listing, displays it in a form and returns the selected item.
    .DESCRIPTION
    The function PopupListSelect takes a listing of items and displays them in a form and then returns the selected item.

    This function has been modified from it's original form located at https://technet.microsoft.com/en-us/library/ff730949.aspx
    .PARAMETER listing
    The list of items to display in the popup window.
    .PARAMETER property
    The name of the property that is actully displayed in the selection box
    .PARAMETER description
    The text on the top of the form articulating what the listing is.
    .PARAMETER cropBegin
    The number of characters to remove at the begining of the listing's property 
    .PARAMETER cropEnd
    The number of characters to remove at the end of the listing's property
    .EXAMPLE
    PopupListSelect $ouListing 'name' 'Please select the Unit:'
    .LINK
    https://technet.microsoft.com/en-us/library/ff730949.aspx
    #>
    
    param ($listing, [string]$property, [string]$description, [int]$cropBegin=0, [int]$cropEnd=0)

    [void] [System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms")
    [void] [System.Reflection.Assembly]::LoadWithPartialName("System.Drawing") 

    $objForm = New-Object System.Windows.Forms.Form 
    $objForm.Text = $description
    ...
    ForEach ($item in $listing) {
        $name = $item.$property
        #Write-Host "$cropEnd + $cropBegin"
        if (($cropBegin -gt 0) -and ($cropEnd -gt 0)) {
            $name = $name.Substring($cropBegin,($name.Length-($cropEnd + $cropBegin)))
        } elseif ($cropBegin -gt 0) {
            $name = $name.Substring($cropBegin)
        }
        [void] $objListBox.Items.Add($name)
    }
    ...
    return $objForm.controls.SelectedItem
...
function PopupBox {
    <#
    .SYNOPSIS
    Function that displays a note to the user
    .DESCRIPTION
    PopupBox displays the heading and message to the user and returns button pressed
    .PARAMETER body
    The main text to dispaly in the popup window.
    .PARAMETER heading
    The header to dispaly in the popup window.
    .PARAMETER button
    The type of button to display.
    .OUTPUTS
    Returns the text of button pressed.
    .EXAMPLE
    PopupBox 'You broke something' 'Error!' 'ok'
    #>
    
    param ([string]$body, [string]$heading,[string]$button="ok")
    ...
    $pressed = $wsShell.Popup($body,0,$heading,$ntype)
...
<#$ouListing = Get-ADOrganizationalUnit -SearchBase 'OU=M.UTOSPA,DC=asurite,DC=ad,DC=asu,DC=edu' -Filter 'Name -like "M.UTOSPA.*"' -SearchScope OneLevel
$unitOU = PopupListSelect $ouListing 'Name' 'Unit OU for the Saved Query:'
if ($unitOU -like "M.UTOSPA.*") {
    $secGroupList = Get-ADGroup -filter "Name -NotLike '$unitOU.Groups.OU*'" -SearchBase "OU=$unitOU.Groups,OU=$unitOU,OU=M.UTOSPA,DC=asurite,DC=ad,DC=asu,DC=edu"
    $unitOU = PopupListSelect $secGroupList 'Name' 'Security Group for the Saved Query:'
    $dlg=New-Object System.Windows.Forms.SaveFileDialog
}#>

function InputPopupBox {
    <#
    .SYNOPSIS
    Function displays input boxes(s) to accept user data
    .DESCRIPTION
    InputPopupBox displays can display multiple input boxes to gather data from the user.
    Majority of the code came from https://technet.microsoft.com/en-us/library/ff730941.aspx
    .PARAMETER feildListings
    A multidimentional array (2*N) containing all the names of the feilds and the prefilled data 
    .OUTPUTS
    A hash table that match the feild to its input and Exit code
    .EXAMPLE
    $feildListings = @('Question 1','Default Data 1'),@('Question 2','Default Data 2')
    InputPopupBox $feildListings
    #>
    
    param ([string[][]]$feildListings)

    if ($feildListings.count -eq 0) {
        $inputs = @{"Status" = "Cancel - No feilds to display"}
    }

    $incLength = $($feildListings.count) * 50

    [void] [System.Reflection.Assembly]::LoadWithPartialName("System.Drawing") 
    [void] [System.Reflection.Assembly]::LoadWithPartialName("System.Windows.Forms") 

    $objForm = New-Object System.Windows.Forms.Form 
    $objForm.Text = "Entry Form"
    $objForm.Size = New-Object System.Drawing.Size(300,(200 + $incLength))
    $objForm.StartPosition = "CenterScreen"

    $objForm.KeyPreview = $True
    $objForm.Add_KeyDown({
        if ($_.KeyCode -eq "Enter") {
            $Script:x = @{"Status" = "Success"}
            if ($feildListings[0].count -eq 1) {
                $Script:x.Add($feildListings[0], $objTextBox[$a].Text)
            } else {
                $a = -1
                foreach ($feild in $feildListings){
                    $a += 1
                    $Script:x.Add($feild[0], $objTextBox[$a].Text)
                }
            }
            $objForm.Close()
        }
    ...
    $objForm.Add_KeyDown({
        if ($_.KeyCode -eq "Escape") {
            $Script:x = @{"Status" = "Canceled"}
            $objForm.Close()
        }
    })

    $OKButton = New-Object System.Windows.Forms.Button
    $OKButton.Location = New-Object System.Drawing.Size(75,(120 + $incLength))
    $OKButton.Size = New-Object System.Drawing.Size(75,23)
    $OKButton.Text = "OK"
    $OKButton.Add_Click({
        $Script:x = @{"Status" = "Success"}
    ...
    $CancelButton = New-Object System.Windows.Forms.Button
    $CancelButton.Location = New-Object System.Drawing.Size(150,(120 + $incLength))
    $CancelButton.Size = New-Object System.Drawing.Size(75,23)
    $CancelButton.Text = "Cancel"
    ...
    foreach ($feild in $feildListings) {
        ...
        $objLabel += New-Object System.Windows.Forms.Label
        $objLabel[$i].Location = New-Object System.Drawing.Size(10,(20+(50*$i))) 
        $objLabel[$i].Size = New-Object System.Drawing.Size(280,20) 
        $objLabel[$i].Text = $feild[0]
        $objForm.Controls.Add($objLabel[$i]) 

        $objTextBox += New-Object System.Windows.Forms.TextBox 
        $objTextBox[$i].Location = New-Object System.Drawing.Size(10,(40+(50*$i))) 
        $objTextBox[$i].Size = New-Object System.Drawing.Size(260,20) 
        $objTextBox[$i].Text = $feild[1]
        $objForm.Controls.Add($objTextBox[$i]) 

    }

    $objForm.Topmost = $True

    $objForm.Add_Shown({$objForm.Activate(); $objTextBox[0].focus()})
    [void] $objForm.ShowDialog()
...
$Treeview_NodeMouseClick=[System.Windows.Forms.TreeNodeMouseClickEventHandler]{
    ...
    if($_.Button -eq 'Right') {
        ...
        if ($_.Node.ImageIndex -lt 3 -and (Get-WmiObject win32_computersystem).partofdomain) {
